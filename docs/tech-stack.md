...
**Decision: PostgreSQL + Supabase** — Relational model is perfect for structured shopping list data (Users → Lists → Items with complex relationships). Supabase provides excellent free tier (500MB, 50k MAU), built-in real-time subscriptions via PostgreSQL CDC, auth, and storage all in one platform. Prisma ORM provides type-safe queries and migrations. **[See detailed database decision rationale](/docs/database/database-decision.md).**
...
- ACID transactions for budget tracking and collaboration consistency
- Complex query support (JOINs, aggregations for price history and trends)
- Row-Level Security (RLS) for multi-tenant data isolation
- Cost: $0/month for MVP (0-500 users), $25/month growth phase (500-5k users)
- Minimal vendor lock-in (standard PostgreSQL, can migrate to AWS RDS or self-host)
