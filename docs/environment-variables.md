...
| `NEXTAUTH_SECRET` | Secret key for session encryption and JWT signing (min 32 chars) | Generate with `openssl rand -base64 32`                      | ✅       |
| `NEXTAUTH_URL`    | Application base URL for NextAuth callbacks                      | `http://localhost:3000` (dev)<br>`https://listly.app` (prod) | ✅       |
| `NODE_ENV`        | Environment mode                                                 | `development` / `production` / `test`                        | ✅       |

**Authentication Setup:**

- `NEXTAUTH_SECRET`: MUST be a strong random string. Never commit to git. Generate a new one for each environment (dev, staging, prod).
- `NEXTAUTH_URL`: Must match your actual URL. In production, use your domain without trailing slash.
- Session strategy: JWT (stateless) with 30-day expiration, updates every 24 hours.
